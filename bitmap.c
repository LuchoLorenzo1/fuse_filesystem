...
void
create_bitmap(bitmap_t *bitmap, int size)
{
	bitmap->bits =
...
bool
is_bit_set(unsigned int *bits, int pos)
{
	int index = pos / 32;
	int bitPos = pos % 32;
...
void
set_bit(unsigned int *bits, int pos)
{
	int index = pos / 32;
	int bitPos = pos % 32;
	unsigned int mask = 1 << bitPos;
...
void
clear_bit(unsigned int *bits, int pos)
{
	int index = pos / 32;
	int bitPos = pos % 32;
	unsigned int mask = ~(1 << bitPos);
...
int
allocate_elements(bitmap_t *bitmap, int numElements)
{
	int count = 0;
	int start = -1;
...
void
remove_elements(bitmap_t *bitmap, int start, int numElements)
{
