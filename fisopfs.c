...
parse_path(const char *path, char *directory, char *filename)
{
	...
	if (last_slash == NULL && last_backslash == NULL) {
		...
		const char *last_separator = (last_slash > last_backslash)
		                                     ? last_slash
		                                     : last_backslash;
...
int
add_dir_entry(const char *filename, int dir_inode, int new_inode)
{
	// if inode.mode != DIR {
	// 	return -1;
	// }
	...
	memcpy(BLOCKS[inode->blocks[0]] + (inode->len * sizeof(dirent_t)),
	       &root,
	       sizeof(dirent_t));
...
int
remove_inode(int inode_index, dirent_t *directory)
{
...
int
remove_dir_entry(const char *filename, int inode_index)
{
	// if inode.mode != DIR {
	// 	return -1;
	// }
...
int
get_inode_index(const char *filename, int inode_index)
{
...
fisopfs_getattr(const char *path, struct stat *st)
{
	...
	st->st_ctime = inode->fecha_de_cambio;
             ...
             struct fuse_file_info *fi)
{
	...
	if (BLOCK_SIZE > offset_size + size) {
		memcpy(buffer,
		       BLOCKS[inode->blocks[offset_blocks]] + offset_size,
		       size);
		return size;
	}
	...
	for (size_t i = offset_blocks + 1; i < n_blocks; i++) {
		memcpy(buffer + offset_size + (i * BLOCK_SIZE),
		       BLOCKS[inode->blocks[i]],
		       BLOCK_SIZE);
	}
	memcpy(buffer + (n_blocks * BLOCK_SIZE),
	       BLOCKS[inode->blocks[n_blocks]],
	       size_left);
              ...
              struct fuse_file_info *fi)
{
	...
	size_t offset_size = offset % BLOCK_SIZE;
	if (BLOCK_SIZE > offset_size + size) {
		memcpy(BLOCKS[inode->blocks[offset_blocks]] + offset_size,
		       buffer,
		       size);
		inode->len += size;
		return size;
	...
	strncpy(BLOCKS[inode->blocks[offset_blocks]] + offset_size,
	        buffer,
	        BLOCK_SIZE - offset_size);
	size_t size_left = size - (BLOCK_SIZE - offset_size);
	...
	for (size_t i = 0; i < n_blocks; i++) {
		memcpy(BLOCKS[block_index + i],
		       (i * BLOCK_SIZE) + buffer,
		       BLOCK_SIZE);
		inode->blocks[inode->size_blocks + i] = block_index + i;
	}
	memcpy(BLOCKS[block_index + n_blocks],
	       (n_blocks * BLOCK_SIZE) + buffer,
	       size_left);
...
static int
fisopfs_link(const char *old_file, const char *new_file)
{
...
static int
fisopfs_utimens(const char *path_archivo, const struct timespec tv[2])
{
	printf("[debug] fisopfs_utimens: %s\n", path_archivo);
	int inode_index = get_inode(path_archivo);
	INODES[inode_index].fecha_de_acceso = tv[0].tv_sec;
	INODES[inode_index].fecha_de_modificacion = tv[1].tv_sec;
	return 0;
}

static int
fisopfs_flush(const char *path_archivo, struct fuse_file_info *f_info)
{
	char cwd[1024];
	char filepath[2048];
